...
syn match p6QPairs contained transparent skipwhite skipempty nextgroup=p6StringQ,p6StringQ_PIR "\%(\_s*:!\?\%([A-Za-z_\xC0-\xFF]\%([A-Za-z_\xC0-\xFF0-9]\|-[A-Za-z_\xC0-\xFF]\@=\)*\)\%(([^)]*)\|\[[^\]]*]\|<[^>]*>\|«[^»]*»\|{[^}]*}\)\?\)*"
...
if !exists("perl6_extended_q") && !exists("perl6_extended_all")
    ...
    let s:after  = "\\%(\\_s*:!\\?\\%([A-Za-z_\\xC0-\\xFF]\\%([A-Za-z_\\xC0-\\xFF0-9]\\|-[A-Za-z_\\xC0-\\xFF]\\@=\\)*\\)\\%(([^)]*)\\|\\[[^\\]]*]\\|<[^>]*>\\|«[^»]*»\\|{[^}]*}\\)\\?\\)*\\_s*\\)\\@<="
